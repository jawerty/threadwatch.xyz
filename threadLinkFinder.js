...
function spawnScraper(thread, retry) {
    ...
    threadScraper.stdout.on('data', (data) => {
        console.log(`stdout for ${thread.threadShortId}: ${data}`);
    });

    threadScraper.stderr.on('data', (data) => {
        console.error(`stderr for ${thread.threadShortId}: ${data}`);
    });
